...
<script type="text/javascript">
	$(document).ready(function(){
		//$("#response").load("<?php  echo base_url('transaksi/amprah/tampil_amprah');?>");
		//$("#tampil").load("myScript.php?var=x&var2=y&var3=z");
		
		/*$("#tampil").load("<?php  echo base_url('tr_keu/tr_bku_saldo/list_data');?>?tgl_awal='<?php  echo date('Y-m-d')?>'&tgl_akhir='<?php  echo date('Y-m-d')?>'");*/

		$("#tampil").load("<?php  echo base_url('tr_keu/tr_bku_saldo/list_saldo_kppn');?>", {
		...
		$("#cari").click(function(){
			...
			var mak = $("#mak").val();
			var sd = $("#sd").val();
			var dt = {'tgl_awal':tgl_awal,'tgl_akhir':tgl_akhir,'bku':bku,'mak':mak,'sd':sd};

			$.ajax({
				url:"<?php  echo base_url('tr_keu/tr_bku_saldo/list_saldo_kppn');?>",
				type:"POST",
				data:dt,
				cache:false,
					...
					<table class="table table-hover">
						<tbody>
							<tr>
								<td>BKU</td>
								<td>
									<input type="text" name="sd" id="sd" class="form-control" value="<?php echo $sd;?>">
							...
							<tr>
								<td colspan="2">
									...
									<a href="<?php echo base_url('tr_keu/tr_bku_saldo/add_bku_saldo')?>" class="btn btn-sm btn-success">
