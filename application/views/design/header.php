...
<head>
  ...
  <link href="<?=base_url();?>assets/dist/css/AdminLTE.min.css" rel="stylesheet" type="text/css" />
    ...
    <link rel="stylesheet" type="text/css" href="<?php echo base_url('assets/css/tautocomplete.css')?>">
    ...
    <!--[if lt IE 9]>
        ...
        <script src="<?php echo base_url();?>assets/js/axios.min.js" type="text/javascript"></script>
        <script src="<?php echo base_url();?>assets/js/jquery.number.js" type="text/javascript"></script>
        <script src="<?php echo base_url();?>assets/js/tautocomplete.js" type="text/javascript"></script>
      ...
      <aside class="main-sidebar">
        ...
        <section class="sidebar">
          ...
          <ul class="sidebar-menu">
            ...
            $s_m = "SELECT * FROM submenu WHERE parent_id=0 and id_roles like '%".$id_role."%' and aktif=1 ORDER BY submenu_id ASC";
            $submenu = mysqli_query($mysqli,$s_m);
            while($dataSubmenu = mysqli_fetch_assoc($submenu)){
              $level2 = "select * from submenu sm where parent_id=".$dataSubmenu['submenu_id']." and id_roles like '%".$id_role."%' and sm.aktif=1 order by sm.submenu_id asc";
                        //echo $level2;
              $s_level2 = mysqli_query($mysqli,$level2);
              if(mysqli_num_rows($s_level2)==0){
                echo '<li><a href="'.base_url($dataSubmenu['submenu_link']).'""><i class="'.$dataSubmenu['menu_icon'].'"></i>'.$dataSubmenu['submenu'].'</a></li>';
              }else{
                echo '<li class="treeview">
                <a href="'.$dataSubmenu['submenu_link'].'">
                  <i class="'.$dataSubmenu['menu_icon'].'"></i> <span>'.$dataSubmenu['submenu'].'</span>
                  <i class="fa fa-angle-left pull-right"></i>
                </a>
                <ul class="treeview-menu">';
                  while($dt_level2 = mysqli_fetch_assoc($s_level2)){
                    $s_level3 = mysqli_query($mysqli,"select * from submenu sm where parent_id=".$dt_level2['submenu_id']." and id_roles like '%".$id_role."%' and sm.aktif=1  order by sm.submenu_id asc ");
                    if(mysqli_num_rows($s_level3)==0){
                      echo '<li><a href="'.base_url($dt_level2['submenu_link']).'""><i class="'.$dt_level2['menu_icon'].'"></i>'.$dt_level2['submenu'].'</a></li>';
                    }
                    else{
                      echo '<li class="treeview">
                      <a href="'.$dt_level2['submenu_link'].'">
                        <i class="'.$dt_level2['menu_icon'].'"></i> <span>'.$dt_level2['submenu'].'</span>
                        <i class="fa fa-angle-left pull-right"></i>
                      </a>
                      <ul class="treeview-menu">';
