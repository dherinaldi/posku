  								...
  								<tbody>
  									<tr>
  										...
  										<td><input type="text" id="kode" class="form-control" value="<?php echo $nota;?>" readonly="true">
  											<input type="hidden" id="id_beli" value="<?php echo $id;?>">
  										</td>
  		...
  		<div class="col-md-4">
  			<div class="box box-success">
  				<div class="box-body">
  					...
  					<div id="total_txt" class="number" style=" font-family: Lucida Console, Courier, monospace; font-size :30px;font-weight: bold;"><?php echo $subtotal;?></div>
  			...
  			<div class="box box-success">
  				<div class="box-header">
  					<div class="box-tools pull-right">
  						<button class="btn btn-info btn-sm" type="button" onclick="addrow()"><i class="fa fa-plus"></i></button>
  					</div>
  				</div>
  				<table class="table table-hover">
  					<thead>
  						...
  						if(isset($detail)){
  							foreach ($detail as $dt) {
  								...
  								<tr id="brs-<?php echo $i;?>">
  									<td>
  										<input type="text" name="barang[]" class="form-control inlinecs" id="barang-<?php echo $i;?>" value="<?php echo $dt->nama;?>">
  										<input type="hidden" name="id_barang[]" id="id_barang-<?php echo $i;?>" value="<?php echo $dt->id_barang; ?>">

  									</td>
  									<td><input type="text" name="qty[]" id="qty-<?php echo $i;?>" class="number inlinecs" onkeyup="hitung(<?php echo $i;?>)" value="<?php echo $dt->qty; ?>"></td>
  									<td><input type="text" name="satuan[]" id="satuan-<?php echo $i;?>" class="inlinecs" value="<?php echo $dt->satuan; ?>"></td>
  									<td><input type="text" name="hbeli[]" id="hbeli-<?php echo $i;?>" class="number inlinecs" value="<?php echo $dt->hbeli; ?>"></td>
  									<td><input type="text" name="batch[]" id="batch-<?php echo $i;?>" class="inlinecs" value="<?php echo $dt->batch; ?>"></td>
  									<td><input type="text" name="expired[]" id="expired-<?php echo $i;?>" class="inlinecs" value="<?php echo date('d-m-Y', strtotime($dt->expired)); ?>"></td>
  									<td><input type="text" name="subtotal[]" id="subtotal-<?php echo $i;?>" class="number inlinecs" value="<?php echo $dt->subtotal; ?>"></td>
  									<td><input type="text" name="dsc_persen[]" id="dsc_persen-<?php echo $i;?>" class="number inlinecs" onkeyup="hitung_dsc(<?php echo $i;?>)" value="<?php echo $dt->diskon_persen; ?>"></td>
  									<td><input type="text" name="dsc_rp[]" id="dsc_rp-<?php echo $i;?>" class="number inlinecs" onkeyup="hitung(<?php echo $i;?>)" onkeypress="eventet(event)" value="<?php echo $dt->diskon_rp; ?>"></td>
  									<td><input type="text" name="total[]" id="total-<?php echo $i;?>" class=" nilai_hasil number inlinecs" value="<?php echo $dt->subtotal; ?>"></td>
  									<td><input type="text" name="hjual[]" id="hjual-<?php echo $i;?>" class="number inlinecs" onkeypress="eventet(event)" value="<?php echo $dt->hjual; ?>"></td>
  									<td>
  										<button type="button" class="btn btn-danger btn-sm" onclick="rembrs(<?php echo $i;?>)"><i class="fa fa-trash"></i></button>
  									</td>
  								</tr>
  								<?php
  					...
  					<div class="table-responsive">
  						<table class="table table-hover">
  							<tbody>
  								<tr>
  									<td>Subtotal</td>
  									<td><input type="text" id="subtotal" class="form-control number" readonly="true" value="<?php echo $subtotal;?>">
  									</td>
  								</tr>
  								<tr>
  									<td>Diskon(%)</td>
...
<script type="text/javascript">
  ...
  var ax = parseInt(baris)+1;
  function isidetail(){
  	for(var i=1;i < ax;i++){
  		autocompleted(i);
  		var nm = $('#barang-'+i).val();
  		$('#tbl_src_barang-'+i).val(nm);
  	}
  }
  ...
  autocompleted(baris);
  setTimeout(function(){ $('#tbl_src_barang-'+baris).focus() },200);
  getev();
 /* getev();
 grandtotal();
 */
